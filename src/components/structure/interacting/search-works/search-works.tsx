...
const SearchWorks: FC = () => {
  const {
    root: { lighttable, searchWorks, ui },
  ...
  const mapFilterGroupItemsToTreeList = (
  ...
  const toggleFilterItemActiveStatus = (
    groupKey: string,
    filterInfoId: string
  ) => {
  ...
  const isActiveFilter =
    ui.sidebarStatus === UISidebarStatusType.MAXIMIZED &&
    ui.sidebarContent === UISidebarContentType.FILTER
      ? "search-works--is-active"
      : "";
  ...
  return (
    ...
    >
      ...
      <div className="search-result-info">
        {hits === 1 && (
          <p>
            <Size size={hits} /> {t("work found")}
          </p>
        )}
        {(hits > 1 || hits === 0) && (
          <p>
            <Size size={hits} /> {t("works found")}
          </p>
        )}
      </div>
      <fieldset className="block keyword-search">
        <TextInput
          ...
          onChange={(allFieldsTerm) =>
            searchWorks.setFreetextFields({ allFieldsTerm })
          }
        ...
        <Toggle
          ...
          onToggle={() =>
            ui.setAdditionalSearchInputsVisible(
              !ui.additionalSearchInputsVisible
            )
          }
        >
          <TextInput
          ...
          <TextInput
            className="search-input"
            label={t("{{catalogWorkReferenceNames}} No.", {
              catalogWorkReferenceNames,
            })}
          ...
          <TextInput
            ...
            onChange={(inventoryNumber) =>
              searchWorks.setFreetextFields({ inventoryNumber })
            }
            onKeyDown={triggerFilterRequestOnEnter}
            onReset={triggerFilterRequest}
            resetable={true}
      ...
      <fieldset className="block">
        <div className="single-filter">
          {/* isBestOf */}
          <span
            className={`filter-info-item ${
              searchWorks.bestOfFilter?.docCount === 0
                ? "filter-info-item__inactive"
                : ""
            }`}
          >
            <Checkbox
            ...
            >
              {t("Masterpieces")}
              <Size size={searchWorks.bestOfFilter?.docCount ?? 0} />
            </span>
          </span>
        </div>
        ...
        <Accordion>
          <Accordion.Entry
            ...
            onToggle={(isOpen) =>
              ui.setFilterItemExpandedState("dating", isOpen)
            }
          >
            <DatingRangeslider
          ...
          {mappedFiltersInfos.map((item) => {
            return (
              <Accordion.Entry
                ...
                onToggle={(isOpen) =>
                  ui.setFilterItemExpandedState(item.id, isOpen)
                }
              >
                <TreeList
                  items={item.children ?? []}
                  isOpenIf={(treeListItem) =>
                    ui.filterItemIsExpanded(treeListItem.id)
                  }
                  ...
                  wrapComponent={(treeListItem, toggle) => (
                    <span
                      className={`filter-info-item ${
                        (treeListItem.data?.count ?? 0) === 0
                          ? "filter-info-item__inactive"
                          : ""
                      }`}
                    >
                      <Checkbox
                        className="filter-info-item__checkbox"
                        checked={searchWorks.selectedFilters.filterGroups
                          .get(treeListItem.data?.groupKey as string)
                          ?.has(treeListItem.id)}
                        onChange={() =>
                          toggleFilterItemActiveStatus(
                            treeListItem.data?.groupKey as string,
                            treeListItem.id
                          )
                        }
                      />
                      ...
                      >
                        {treeListItem.name}
                        <Size size={treeListItem.data?.count} />
                      </span>
                    </span>
                  )}
                ></TreeList>
